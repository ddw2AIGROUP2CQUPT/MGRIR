...
from models.model_all_glcm import *
...
parser.add_argument('--model_name', type=str, help="the model name", default='dap1_slic2002_bS128_lrdecay_Netchan5_glcm_adam')
...
class MyData(Data):
    def __init__(self, x=None, edge_index=None, edge_attr=None, y=None, glcm=None):
...
class MyDataset(Dataset):
    def __init__(self, data_dir, split='train'):
        ...
        self.transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((self.glcm_mean,), (self.glcm_std,))
        ])

        
    def calculate_glcm_mean_std(self):
        glcm_means = []
        glcm_stds = []
        for idx in range(len(self)):
            with h5py.File(os.path.join(self.data_dir, self.split, self.filenames[idx]), 'r') as f:
                glcm = np.array(f['glcm'])
                glcm_means.append(np.mean(glcm))
                glcm_stds.append(np.std(glcm))
        mean = np.mean(glcm_means)
        std = np.mean(glcm_stds)
        return mean, std
...
dataloader = DataListLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
dataloader_test = DataListLoader(dataset_test, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
...
def train(args):
    ...
    while global_step < total_steps:
        ...
        for step, sample_batched in enumerate(dataloader):
            ...
            if global_step % args.test_freq == 0:  # test every 'args.test_freq' steps and record the test log
                model.eval()
                ...
                with torch.no_grad():
                    ...
                    # computing Recall、AUC and F1-score
